...
export class NavPanel extends React.Component<NavPanelProps, NavPanelState> {
  ...
  componentDidMount() {
    setState("matthewbraun.ca.nav.isShown", false, "volatile")
    subscribeToState("matthewbraun.ca.nav.isShown", "navPanel", (isShown: boolean) => {
        ...
        {this.state.isShown || !this.state.animateFinished ?
          <NavPanelContainer isShown={this.state.isShown}>
            {window.innerWidth < 800 ?
              <BackButtonContainer onClick={() => { setState("matthewbraun.ca.nav.isShown", false, "volatile") }}>
